...
def main():
    ...
    df.select('VehicleNumber').distinct().groupBy().count()\
        .write.mode("overwrite").json('hdfs://localhost:8020/user/ficp/projekt/analysis/number_vehicles_lines.json')
    
    # 2. Number of records per hour
    df.withColumn('hour',F.hour('Time')).groupBy('hour').count().orderBy(F.asc('hour'))\
        .write.mode("overwrite").json('hdfs://localhost:8020/user/ficp/projekt/analysis/number_of_records.json')
    ...
    df.withColumn('hour',F.hour('Time')).groupBy('hour').agg(F.countDistinct('VehicleNumber')).orderBy(F.asc('hour'))\
        .write.mode("overwrite").json('hdfs://localhost:8020/user/ficp/projekt/analysis/number_of_buses_per_hour.json')

    # 4. Number of buses records per weather update
    df.groupBy('last_updated_epoch').count().orderBy('count')\
        .write.mode("overwrite").json('hdfs://localhost:8020/user/ficp/projekt/analysis/buses_per_weather_update.json')
    ...
    df.select(['Lines','VehicleNumber']).groupBy('Lines').agg(F.countDistinct('VehicleNumber')).orderBy(F.desc('count(VehicleNumber)'))\
        .write.mode("overwrite").json('hdfs://localhost:8020/user/ficp/projekt/analysis/number_of_buses_per_line.json')
    ...
    for b in [7253, 15617, 80635, 7739, 4211]:
        df.filter(F.col('VehicleNumber') == b).filter(F.col('Time')>from_d)\
            .write.mode("overwrite").json(f'hdfs://localhost:8020/user/ficp/projekt/analysis/bus_{b}_route.json')
    
    # 7. Fastest buses
    df.groupBy('VehicleNumber').agg(F.avg('speed')).orderBy(F.desc('avg(speed)'))\
        .write.mode("overwrite").json('hdfs://localhost:8020/user/ficp/projekt/analysis/fastest_buses.json')
    
    # 8. Fastest lines
    df.groupBy('Lines').agg(F.avg('speed')).orderBy(F.desc('avg(speed)'))\
        .write.mode("overwrite").json('hdfs://localhost:8020/user/ficp/projekt/analysis/fastest_lines.json')

    # 9. Precip
    df.withColumn('hour',F.hour('Time')).dropna().groupBy('hour').agg(F.avg('speed'),F.avg('precip_mm')).orderBy(F.desc('hour'))\
        .write.mode("overwrite").json('hdfs://localhost:8020/user/ficp/projekt/analysis/speed_precip.json')

    # 10. Temp
    df.withColumn('hour',F.hour('Time')).dropna().groupBy('hour').agg(F.avg('speed'),F.avg('temp_c')).orderBy(F.desc('hour'))\
        .write.mode("overwrite").json('hdfs://localhost:8020/user/ficp/projekt/analysis/speed_temp.json')
    ...
    df.groupBy(['agg_Lat','agg_Lon','key']).count().orderBy(F.col('key'))\
        .write.mode("overwrite").json('hdfs://localhost:8020/user/ficp/projekt/analysis/heatmap.json')
